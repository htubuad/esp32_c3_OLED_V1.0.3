...
esp_err_t temp_sensor_init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "temperature_sensor_install failed: %s", esp_err_to_name(ret));
        ESP_LOGW(TAG, "ESP32-C3 no internal temp sensor, using simulated value");
        s_available = false;
        return ESP_OK;
    }
    ret = temperature_sensor_enable(s_tsens);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "temperature_sensor_enable failed: %s", esp_err_to_name(ret));
